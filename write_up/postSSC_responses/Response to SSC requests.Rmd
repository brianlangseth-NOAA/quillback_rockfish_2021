...
Additional length data for quillback rockfish were provided by California Department of Fish and Wildlife (CDFW) on July 12, 2021. These data were collected by observers onboard commercial passenger fishing vessels (CPFV) fishing in central California (Reilly et al. 1998) from 1987-1998, and were not available for use in the base model that was reviewed and adopted in the June Groundfish Subcommittee meeting of the Scientific
and Statistical Committee (GFS-SSC). A total of 753 additional length observations from 1987-1998 (Table \ref{tab:len_data}) were added to the California quillback rockfish base model for the recreational fleet.

Length distributions from the new data were similar to existing length distributions within the adopted base model (Figure \ref{fig:sens-debWV-comp}). Given the use of new data, scenarios with and without data weighting were run. Changes to data weights were minimal and resulted in little change in model trajectories. Spawning output (Figure \ref{fig:sens-debWV-ssb}), depletion (Figure \ref{fig:sens-debWV-depl}), and recruitment deviations (Figure \ref{fig:sens-debWV-rdevs}) from models with the new data were similar to those from the adopted base model. Parameter estimates, derived quantities, and likelihood components for each scenario are provided in Table \ref{tab:sens-table}.
...
plot_info = read.csv(file.path(mod_dir, "8_0_0_debWV", "plots", "plotInfoTable_19-07-2021_13.24.22.csv"))
capt = plot_info[which(plot_info$file == "comp_lendat_flt2mkt0_page1.png"),"caption"]
capt2 = paste(substr(capt,1,61), substr(capt,70, 135))

add_figure(
filein = file.path(mod_dir, "8_0_0_debWV", "plots", "comp_lendat_flt2mkt0_page1.png"),
caption = paste(capt2, "White shading indicates the length distributions of the new data in years (1987-1989, 1993-1998) that overlap with original data (gray shading). For 1990-1992, which do not overlap with the original data, the gray shading is the length distributions of the new data"),
...
An alternative catch history was explored in the original California quillback rockfish report (see sensitivity #14 on page 18). The average catch in the three years before and three years after years with high catch were used in the alternative catch time series. Catch was higher than neighboring years in 1991 for the commercial fleet, and 1983 and 1993 for the recreational fleet. Alternative catch values were therefore applied to these years. For the results of this sensitivity, see Table 13, and Figures 47-49 from the California report.

No new catch values were provided by CDFW for this document. 
...
We explored numerous options for blocking selectivity based on changes in the estimated percent of area open to fishing (Table A-1 from CDFW Report 1), as well as changes in mean length from the length data in the adopted base model (Figure 20 from the California quillback rockfish report). Specific blocking along with the reasons for blocking (in parenthsis) are described below. 
...
\begin{enumerate}
  ...
  \item Blocking selectivity from 1916-2000 assuming asymptotic selectivity, 2001-2003 allowing for dome-shaped selectivity, 2003-2007 allowing for dome-shaped selectivity, 2008-2016 allowing for dome-shaped selectivity, and 2017-2020 allowing for dome-shaped selectivity (due to depth restrictions starting in 2001, changes in the percent of area open to fishing in 2003 and again in 2008, and a relaxation of depth restrictions starting in 2017).
...
Data weights were not reweighted for any of the selectivities from this request, in keeping with typical practices when running sensitivities. Suggested reweighted values were within 5% of the original weights from the adopted base model. Under option 2, the selectivity parameter (parameter 2) controlling the width of the peak was hitting its lower bound, which was resolved by fixing this parameter at its lower bound.  

Additional runs here also show limited support for the additional complexity in blocking selectivity. Spawning output (Figure \ref{fig:sens-block-ssb}) and depletion (Figure \ref{fig:sens-block-depl}) from models with alternative blocking for the recreational fleet were similar to those from the adopted base model. Selectivities for both the recreational fleets for each scenario are shown in Figure \ref{fig:sens-block-selex}. Other parameter estimates, derived quantities, and likelihood components for each scenario are provided in Table \ref{tab:sens-table}. 
...
The two requested runs were completed in the original California quillback rockfish report (sensitivities #4 and #6 on page 17). Results for those sensitivities are shown in Table 13 and Figures 47-49 in the California report and so are not repeated here.   
