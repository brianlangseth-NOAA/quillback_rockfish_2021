...
Estimated parameter values are provided in Table \ref{tab:params}. The $R_0$ was estimated at `r round(model$parameters[model$parameters$Label == 'SR_LN(R0)', 'Value'], 2)`. The selectivity curves for the commercial and recreational fleet are shown in Figure \ref{fig:selex}. Selectivity was fixed to be asymptotic with a peak in maximum selectivity for the commercial fleet starting at `r round(model$parameters[model$parameters$Label == 'Size_DblN_peak_WA_Commercial(1)', 'Value'], 1)` cm and a peak in maximum sleectivity for the recreational fleet starting at `r round(model$parameters[model$parameters$Label == 'Size_DblN_peak_WA_Recreational(1)', 'Value'], 1)`. Sensitivities to the selectivity shape and potential exclusion of sparse years of length data for both the commercial and recreational fleets were explored (see below in the [Sensitivity Analyses](#sensitivity-analyses) section). 
