...
The negative log-likelihood profile across natural mortality supported the largest values in the profile near twice the fixed value of `r model$parameters[model$parameters$Label == "NatM_p_1_Fem_GP_1","Value"]` (Figure \ref{fig:m-profile}).
...
The two sensitivities that had the largest impact relative to the base model were the ones that estimated $L_{\infty}$ and the coefficient of variation in length of older fishes (Figures \ref{fig:sens-ssb} and \ref{fig:sens-depl}). These two sensitivities estimated log(R0) 50 and 75 percent larger than the base model despite a change in $L_{\infty}$ of 2 cm (from `r model$parameters[model$parameters$Label == "L_at_Amax_Fem_GP_1","Value"]`), and change in the coefficient of variation in length of older fishes to around 7 percent (from 10 percent).
