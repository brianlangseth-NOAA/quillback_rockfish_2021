...
Similar to results for $R_0$, the profile on steepness suggests the data have limited information on the value of steepness. Value of steepness between 0.3-0.9 were supported based on change in log-likelihood, with the lowest negative log-likelihood occurring near a value of 0.9, which is greater than the value for the base model of `r round(model$parameters[model$parameters$Label == "SR_BH_steep",3],2)` (Figure \ref{fig:h-profile}). Assuming lower steepness values resulted in higher estimates of unfished and recent spawning output (Figure \ref{fig:h-ssb}) and reduced levels of depletion relative to unfished conditions, though relative final stock status was within the management precatuionary zone (0.25 - 0.40 of unfished biomass) across steepness values except for the lowest values (0.30 and 0.40) (Figure \ref{fig:h-depl}).
