...
The 2021 assessment of `r spp` (_`r spp_latin`_) in California waters documented that the population of `r spp` was below the Minimum Stock Size Threshold (MSST), which is 25% of unfished spawning output for rockfish stocks, in 2021. The population declined below MSST starting in 1992, reached it lowest values in the mid-1990s but increased to near the MSST in the 2000s and early 2010s, and declined in recent years [@Langseth_status_2021]. The stock is expected to be declared overfished for 2023 in 2021. Given the assumed productivity of the stock combined with the longevity of `r spp` a range of alternative rebuilding approaches were examined with rebuilding ranging from `r reb[[1]]$recovery_spec[3] + reb[[1]]$tmin ` to `r reb[[1]]$tmax ` based on various SPR harvest rates from `r round(min(reb[[1]]$rebuild_quants[,2], reb[[2]]$rebuild_quants[,2]), 2)` to `r round(max(reb[[1]]$rebuild_quants[,2], reb[[2]]$rebuild_quants[,2]), 2)` (no harvest). 
