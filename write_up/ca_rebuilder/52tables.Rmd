...
row_names = c("2021 Assumed Removals (mt)", 
			  ...
			  "2025 ACL (mt)", "SPR in 2025", "T\\textsubscript{TARGET}", 
			  "T\\textsubscript{MAX}",
...
out4 = as.data.frame(append(out3, list("Ramp SPR= .600" = ramp60[,4]), after = 4))
out5 = as.data.frame(append(out4, list("Ramp SPR= .700" = ramp70[,6]), after = 7))
...
table_format(x = out5,
             caption = "Catches (mt) by year for rebuilding strategies, including ramp strategies. Catches in 2021-2022 for all strategies were set at values recommended by the Groundfish Management Team. Catches in 2023-2024 for ramp strategies were fixed to approximate a linear ramp in catch for 2023-2025.",
             label = "acl-mat",
             align = 'l',
             digits = 3,
             col_names = c(colnames(out)[1:2], 
...
out5[,-c(1:2)] = round(out5[,-c(1:2)]/buffer[,2],2)
