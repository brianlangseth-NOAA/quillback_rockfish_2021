...
#Because ACLs no longer equal OFLs the OFLs for no-cap scenarios are less than acl's. Need to set values for the SPR = 0.5 run and ABC catch run to be from acl_matrix
out[,2] = round(reb[[2]]$acl_matrix[1:46, 2],2) #Set the SPR = 0.5 run to be from the no-ABC-cap run
out[,10] = round(reb[[2]]$acl_matrix[1:46, 10],2) #Set the Tmid run to be from the no-ABC-cap run
...
out[,3:9] = round(out[,3:9]/buffer[,2],2)
table_format(x = out,
