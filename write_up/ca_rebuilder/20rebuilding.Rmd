...
The sum of ACL contributions for `r spp` in California waters from the southern and northern management areas for 2024 was larger than the catch resulting from applying an SPR of 0.5 in 2025. This implies that an SPR rate of below 0.5 would be needed to achieve the current ACL contributions, so results from this strategy (strategy 2 above) are not provided in this report. 
<!-- Similarly, the SPR rate that led to a 50$\%$ probability of recovery by $\text{T}_\text{MAX}$ from the current cycle was less than 0.5 so results from the $\text{T}_\text{MAX}$ strategy (strategy 5a above) are not presented in this report. -->
