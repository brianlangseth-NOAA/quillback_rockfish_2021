...
Potential alternative states of nature were explored for this assessment following guidance in the Terms of Reference, and included low and high ln({\tagstructbegin{tag=Formula}\tagmcbegin{tag=Formula}\(R0\)\leavevmode\tagmcend\tagstructend}), low and high M, and high and low {\tagstructbegin{tag=Formula}\tagmcbegin{tag=Formula}\(L_{\infty}\)\leavevmode\tagmcend\tagstructend}. Alternative values of ln({\tagstructbegin{tag=Formula}\tagmcbegin{tag=Formula}\(R0\)\leavevmode\tagmcend\tagstructend}) covered a wider range of spawning output and depletion estimates compared to the other states of natures and therefore was used. Other states of nature also had difficulty with convergence and not matching the desired spawning output quantiles. The values of ln({\tagstructbegin{tag=Formula}\tagmcbegin{tag=Formula}\(R0\)\leavevmode\tagmcend\tagstructend}) that resulted in spawning ouput for the lower and upper 75\% confidence interval based on the estimated uncertainty in the base model around 2021 spawning output were 2.01 (low) and 2.28 (high). The same catch values from the base model projections were applied to the low and high states of nature to provide a range of projected catch quantities given assumptions about alternative states of ln({\tagstructbegin{tag=Formula}\tagmcbegin{tag=Formula}\(R0\)\leavevmode\tagmcend\tagstructend}) (Table \ref{tab:decision-table}).
...
\caption{\label{tab:decision-table}Decision table for low (2.01) and high (2.28) states of nature around ln(R0).}\\
\toprule
...
\midrule
\endfirsthead
\caption[]{\label{tab:decision-table}Decision table for low (2.01) and high (2.28) states of nature around ln(R0). \textit{(continued)}}\\
\toprule
Year & Catch (mt) & Low: Spawning Output & Low: Fraction Unfished & Base: Spawning Output & Base: Fraction Unfished & High: Spawning Output & High: Fraction Unfished\\
\midrule
\endhead
